...
router.post('/employees', requireRole(['owner','admin','super-admin']), createEmployee);

// ───────────────── Employee Management ─────────────────
// ✅ Update employee schedule (allow self employee + owner/admin)
router.put('/:id/employee/schedule', requireRole(['owner','admin','barber','super-admin']), updateEmployeeSchedule);

// Update employee services (owner/admin)
router.put('/:id/employee/services', requireRole(['owner','admin','super-admin']), updateEmployeeServices);

// ✅ Blocks management (employee self + owner/admin)
router.get('/:id/employee/blocks',    requireRole(['owner','admin','barber','super-admin']), listEmployeeBlocks);
router.post('/:id/employee/blocks',   requireRole(['owner','admin','barber','super-admin']), addEmployeeBlock);
router.delete('/:id/employee/blocks/:blockId', requireRole(['owner','admin','barber','super-admin']), deleteEmployeeBlock);

// Role & Status
router.put('/:id/role',   requireRole(['owner','admin','super-admin']), updateUserRole);
router.put('/:id/status', requireRole(['owner','admin','super-admin']), toggleUserActive);
