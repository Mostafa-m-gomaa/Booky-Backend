...
const verifyOtp = async (req, res, next) => {
  try {
    ...
    if (!user.otp?.hash || !user.otp?.expiresAt || now > user.otp.expiresAt) {
      return res
        .status(400)
        .json({ message: "الكود منتهي الصلاحية. اطلب كودًا جديدًا." });
    }
    ...
    if (user.otp?.lockedUntil && now < user.otp.lockedUntil) {
      ...
      return res
        .status(429)
        .json({ message: `الحساب مقفول مؤقتًا. جرّب بعد ${waitMin} دقيقة.` });
    }
...
const resendOtp = async (req, res, next) => {
  try {
    ...
    if (
      user.otp?.lastSentAt &&
      now - user.otp.lastSentAt < OTP_RESEND_SECONDS * 1000
    ) {
      const wait = Math.ceil(
        (OTP_RESEND_SECONDS * 1000 - (now - user.otp.lastSentAt)) / 1000
      );
      return res
        .status(429)
        .json({ message: `من فضلك انتظر ${wait} ثانية قبل طلب كود جديد.` });
    }
    ...
    try {
      ...
      return res
        .status(502)
        .json({ message: "تعذّر إرسال كود التفعيل عبر واتساب. حاول لاحقًا." });
    }
